...
int answ(int arr[], int i, int j)
{
    ...
    for (int k = i; k <= j-1; k++)
    {
        int temp = answ(arr, i, k) + answ(arr, k + 1, j) + (arr[i-1] * arr[k] * arr[j]);
        cout << temp << endl;
        if(temp < min_1){
            min_1 = temp;
        }
...
void solution()
{
    ...
    /*     cin >> arr[n]; */
    /* } */
    /* cout << answ(arr,1,n-1) << endl; */

    /* int k,n,root; */
    /* cin >> n >> k >> root; */
    /* adj_mat_for_graph(k,n); */
    /* bfs(root,n,k); */
    /* dfs_int(root,n,k); */
    ...
    vector<int> room_query(r);
    for(int i =0;i<r;i++){
        cin >> room_query[i];
    }
    sort(applic.begin(),applic.end());
    sort(room_query.begin(),room_query.end());
    int anss = 0;
    while(app < a && room < r){
        if(abs(applic[app]-room_query[room]) <= d){
            anss++;
            app++,room++;
        }
        else{
            if(applic[app] > room_query[room]){
                room++;
            }
            else{
                app++;
