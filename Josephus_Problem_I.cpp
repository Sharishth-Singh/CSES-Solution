...
/* ================================================================== Solution Start ============================================================== */
vector<int>ans;
void fun(ordered_set se,int posi){
   /* if(se.size() == 0){ */
   /*    return; */
   /* } */
   /* ans.push_back(se.find_by_order(( posi+1 )%se.size())->first); */
   /* se.erase(se.find_by_order(( posi+1 )%se.size())); */
   /* if(se.size() == 0){ */
   /*    return; */
   /* } */
   /* /1* std::cerr << posi  << " before "  << se.size()<< endl; *1/ */
   /* posi = ( posi+1 )%( se.size()+1 ); */
   /* /1* std::cerr << posi  << " after " << endl; *1/ */
   /* fun(se,posi); */
